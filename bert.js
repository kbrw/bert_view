...
BertClass.prototype.simpleterm = function(term) {
  var r, v, _i, _j, _len, _len1, _results;
  if (term.length > 0 && term.type !== "Tuple" && term[0].type === "Tuple" && term[0].length === 2 && term[0][0].type === "Atom") {
    r = {};
    for (_i = 0, _len = term.length; _i < _len; _i++) {
      v = term[_i];
      r[v[0]] = this.simpleterm(v[1]);
    }
    return r;
  } else if (term.length) {
    _results = [];
    for (_j = 0, _len1 = term.length; _j < _len1; _j++) {
      v = term[_j];
      _results.push(this.simpleterm(v));
    }
    return _results;
  } else if (term.value){
    return term.value;
